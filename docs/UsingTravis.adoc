...
.  On GitHub, create a new user account and give this account collaborator and admin access to the repo. +
   Using this account, generate a Employeeal access token https://github.com/settings/tokens/new[here].
+
[NOTE]
Employeeal access tokens are like passwords so make sure you keep them secret! If the Employeeal access token is leaked, please delete it and generate a new one.
+
[NOTE]
...
* Click `Generate Token` and copy your new Employeeal access token.
--
...
--
* name: `GITHUB_TOKEN`
* value: Employeeal access token copied in step 1
...
Otherwise, other people will be able to see the Employeeal access token and thus have access this repo. +
